...
def load_habits():
    try:
        ...
        # Mantém apenas as 11 colunas oficiais
        ...
        # Conversão estrita para datetime.date garantindo ordenação perfeita
...
FRASES_MOTIVACIONAIS = [
    ...
    "O cigarro não resolve a tensão, ele apenas cria a abstinência do próximo maço.",
...
with tab_semana:
    ...
    if df.empty:
        st.info("Sua planilha está pronta. Cole os dados na aba habits!")
    else:
        # Ordenação decrescente garantida das semanas
        todas_semanas = sorted(list(set(df['semana_inicio'].dropna().tolist())), reverse=True)
        semanas_dict = {
    ...
    with st.form("form_dia_fechado"):
        ...
        if btn_gravar_dia:
            ...
            if novas_linhas:
                ...
                st.warning("Preencha pelo menos um hábito para salvar.")
...
# -------------------------------------------------------------
with tab_novo:
    st.header("Novo Registro Avulso")
...
# -------------------------------------------------------------
with tab_editar:
    st.header("Gerenciador de Linhas")
    if df.empty:
